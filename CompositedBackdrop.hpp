...
#include "GlassReflectionBackdrop.hpp"
#include "CanvasGeometry.hpp"

namespace AcrylicEverywhere
{
	enum class BackdropType : UCHAR
	{
		None,
		Acrylic,
		Mica,
	...
	{
		uDwmPrivates::CTopLevelWindow* window;

		CNCVisualManager(uDwmPrivates::CTopLevelWindow* topLevelWindow = nullptr) : window{topLevelWindow} {}
		void UpdateCloak(bool cloak)
		{
			...
			uDwmPrivates::CVisual* legacy{ nullptr };
			uDwmPrivates::CVisual* accent{ nullptr };
			uDwmPrivates::CVisual* clientBlur{ nullptr };
			uDwmPrivates::CVisual* systemBackdrop{ nullptr };
			uDwmPrivates::CVisual* accentColor{ nullptr };
	...
	private:
		...
		BackdropType m_type{ BackdropType::None };
		bool m_enableGlassReflection{ false };
		bool m_enableBorders{ true };
		...
		winrt::com_ptr<DCompPrivates::IDCompositionDesktopDevicePartner> m_interopDCompDevice{ nullptr };
		winrt::com_ptr<uDwmPrivates::CCanvasVisual> m_borderVisual{ nullptr };
		winrt::com_ptr<uDwmPrivates::CSolidColorLegacyMilBrushProxy> m_milBrush{ nullptr };
		uDwmPrivates::CTopLevelWindow* m_window{ nullptr };
		uDwmPrivates::CTopLevelWindow* m_sourceWindow{ nullptr };
		uDwmPrivates::CWindowData* m_windowData{ nullptr };
		...
		uDwmPrivates::CTopLevelWindow* GetSourceWindow() const
		{
			return m_sourceWindow ? m_sourceWindow : m_window;
		}
		uDwmPrivates::CTopLevelWindow* GetTargetWindow() const
		{
			return m_window;
		}
		...
		{
			THROW_IF_FAILED(
				uDwmPrivates::CCanvasVisual::Create(
					m_borderVisual.put()
				)
			);
	...
	public:
		...
		CCompositedBackdrop(uDwmPrivates::CTopLevelWindow* window) : 
			m_window{ window },
			m_windowData{ window->GetWindowData() }, 
			m_sharedVisual{},
		...
		CCompositedBackdrop(
			...
			uDwmPrivates::CTopLevelWindow* dest
		) : 
			m_window{ dest },
			...
			m_sharedVisual{}
		{
			...
			{
				UpdateBackdropType(allowNCBackgroundVisual ? backdrop->m_type : BackdropType::None);
		...
		HRESULT UpdateClipRegion(HRGN hrgn) try
		{
			...
			{
				winrt::com_ptr<uDwmPrivates::CRgnGeometryProxy> geometry{nullptr};
				THROW_IF_FAILED(
		...
		{
			auto visualCollection{ m_sharedVisual.layerVisual.Children() };
			if (!m_backdrop && m_type != BackdropType::None)
			{
				switch (m_type)
				{
		...
		HRESULT UpdateBorderVisual() try
		{
			if (m_enableBorders)
			{
				...
				if (
					(
						uDwmPrivates::CDesktopManager::s_pDesktopManagerInstance->IsVanillaTheme() &&
				...
				)
				{
					...
					{
						winrt::com_ptr<uDwmPrivates::CDrawGeometryInstruction> instruction{ nullptr };
						THROW_IF_FAILED(
							uDwmPrivates::CDrawGeometryInstruction::Create(
								m_milBrush.get(),
								geometry.get(),
								instruction.put()
							)
	...
	private:
		...
		uDwmPrivates::ACCENT_POLICY m_accentPolicy{ 6 };

		winrt::com_ptr<DCompPrivates::IDCompositionDesktopDevicePartner> m_interopDCompDevice{ nullptr };
		uDwmPrivates::CTopLevelWindow* m_window{ nullptr };
		uDwmPrivates::CTopLevelWindow* m_sourceWindow{ nullptr };
		winrt::com_ptr<uDwmPrivates::CBaseGeometryProxy> m_clipRegion{ nullptr };
		...
		bool IsContainShadow()
		{
			return 
				(
					(m_accentPolicy.AccentFlags & (1 << 5)) ||
					(m_accentPolicy.AccentFlags & (1 << 6)) ||
					(m_accentPolicy.AccentFlags & (1 << 7)) ||
					(m_accentPolicy.AccentFlags & (1 << 8))
				);
		}
	public:
		const CSharedVisual& GetSharedVisual() const
		...
		CCompositedAccentBackdrop(uDwmPrivates::CTopLevelWindow* window) :
			m_window{ window },
			m_windowData{ window->GetWindowData() },
			m_sharedVisual{},
			m_ncVisualManager{ window }
		...
		CCompositedAccentBackdrop(const CCompositedAccentBackdrop* backdrop, uDwmPrivates::CTopLevelWindow* dest) :
			m_window{ dest },
			m_sourceWindow{ backdrop->m_window },
			m_windowData{ backdrop->m_windowData },
			m_sharedVisual{}
		{
			...
			{
				UpdateBackdropType(allowAccentVisual ? backdrop->m_type : BackdropType::None);
				UpdateGlassReflectionState(backdrop->m_enableGlassReflection);
		...
		{
			if (memcmp(&m_accentPolicy, m_windowData->GetAccentPolicy(), sizeof(uDwmPrivates::ACCENT_POLICY)) != 0)
			{
